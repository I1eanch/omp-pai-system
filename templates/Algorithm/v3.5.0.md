...
- Write PRD.md with Write tool — include YAML frontmatter (task, slug, effort, phase, progress, mode, started, updated) + sections (Context, Criteria, Decisions, Verification) per `PRDFORMAT.md` under the PAI template root from `PORTABLE ROOT MAP`
...
4. `STATE/work.json` under the MEMORY root from `PORTABLE ROOT MAP` has the registry of all sessions (populated by read-only PRDSync + PRDStateSync hooks)
...
**Full spec:** `PRDFORMAT.md` under the PAI template root from `PORTABLE ROOT MAP` (read during OBSERVE if needed for field details or continuation rules).
